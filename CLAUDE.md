...
Servers are sent as `EventSubscriptionList` (groups by subscription URL) rather than the old flat `EventServerList`. `CmdPing`, `CmdAddSub`, `CmdRemoveSub` are also defined.
...
**TUI** (`internal/tui`): Bubbletea app with three panels — server list (left, `TabsPanel`), server details (right), log output (bottom). `TabsPanel` groups servers by subscription with left/right tab navigation and up/down server navigation. Servers have absolute indices across all groups. IPC events drive model updates via `tea.Cmd` wrappers.
...
State (last selected server index + name) is persisted to `~/.cache/nwxraytui/state.json` via `config.LoadState`/`SaveState`.
...
`latency.PingDirect` sets SO_MARK (Linux fwmark) on the socket to bypass TUN routing; `latency.Ping` uses the default path.
